...
function Line({ label, children, aside }) {
  return (
    <div className="border-hair-paper grid gap-1 border-t px-5 py-3">
      ...
      {aside && <p className={`${MONO_LABEL} ${MEASURE} text-paper-faint`}>{aside}</p>}
    </div>
  )
}
    ...
    <>
      ...
      <div className="console-side-body">
        <div className="border-hair-paper grid gap-2 border-b px-5 py-3">
          <label className={`${MEASURE} relative block`}>
            <Search
              aria-hidden="true"
            ...
            <div className="border-hair-paper [&:not(:first-child)]:border-l">
              ...
              {facts.length > 0 && (
                <section>
                  ...
                  {/* One per row. A metric brings its own gutter and its own
                      rule between neighbours, and a column of them reads as the
                      reference card it is. */}
                  <dl>
                    {facts.map(fact => (
                      <Metric
                        key={fact.id}
