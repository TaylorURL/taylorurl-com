-- Migration: add missing tables and triggers
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'profiles' AND policyname = 'Service role can manage all profiles') THEN
    CREATE POLICY "Service role can manage all profiles" ON public.profiles FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'websites' AND policyname = 'Service role can manage all websites') THEN
    CREATE POLICY "Service role can manage all websites" ON public.websites FOR ALL USING (true) WITH CHECK (true);
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'website_stats' AND policyname = 'Service role can manage all website stats') THEN
    CREATE POLICY "Service role can manage all website stats" ON public.website_stats FOR ALL USING (true) WITH CHECK (true);
  END IF;
END $$;
