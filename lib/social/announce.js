 ...
 */
import { CADENCE, HORIZON_DAYS, connect, freeSlots, place, posts, wiring } from './buffer.js'
...
const TEMPLATES = {
  ...
  googlebusiness: article => [
    ...
    'Sites are built, hosted and looked after from Baytown, so a first meeting is a drive ' +
      'across town rather than a call. Tell me what your business sells and who buys it, and ' +
      'a plan and a price come back.',
...
function sentences(text) {
  return (text.match(/[^.!?]+[.!?]+(?:\s|$)/g) ?? [text]).map(sentence => sentence.trim())
}
...
export function postText(service, article) {
  ...
  for (let take = kept.length; take >= 0; take -= 1) {
