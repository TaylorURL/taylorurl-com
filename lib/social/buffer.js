...
/** Where a post's links send the reader, and where cards are served from. */
...
// Buffer keeps a post it could not publish and answers no mutation that retries
// one, so taking it out is the only thing the API does to a failed post
// directly. `EDIT` onto a free slot is the other half of what one needs.
const DELETE = `
...
export function runwayDays(lastDue, now) {
  if (!lastDue) return 0
  return Math.max(Math.floor((new Date(lastDue).getTime() - now.getTime()) / DAY), 0)
}

/**
 * Whether Buffer is holding a post because it tried to publish it and could not.
 *
 * Two names for one state. The API has answered both `error` and `failed` on a
 * post that did not go out, and which of them arrives is not the caller's to
 * know, so everything that acts on a failed post asks here rather than naming
 * one of the words and quietly missing the other.
 */
export const isFailed = post => post.status === 'error' || post.status === 'failed'
...
/**
 * New words on posts already in the queue, keeping everything else about them.
 *
 * A post's wording outlives the day it was written on. The Page carries a month
 * of runway, so a change in how the studio talks about itself reaches a reader
 * five weeks late unless the posts already scheduled are rewritten too, and by
 * then they have published in the old voice thirty times.
 *
 * `editPost` replaces a post rather than patching it, which is the whole
 * difficulty: an edit that names only the text drops the image off an Instagram
 * post, the button off a Business Profile post, and the day off both. So every
 * other field is read back off the post first and sent again unchanged, and a
 * post the queue does not hold is refused rather than created.
 *
 * The wiring and the queue are read once for the whole batch, so rewriting
 * thirty posts costs thirty-two requests rather than ninety.
 *
 * @param {string|object} key A Buffer public API key, or a run from `connect`.
 * @param {object[]} edits Each `{ id, text }`, naming a post Buffer holds.
 * @param {object} [options]
 * @param {boolean} [options.dryRun] Read and match, and write nothing.
 * @returns {{rewritten: object[], missing: string[], unchanged: string[]}}
 */
export async function rewrite(key, edits, { dryRun = false } = {}) {
  const run = connect(key)
  const { organizationId, channels } = await wiring(run)
  const all = await posts(run, organizationId)
  const held = new Map(all.map(post => [post.id, post]))
  const service = new Map(channels.map(channel => [channel.id, channel]))
  ...
  try {
    for (const edit of edits) {
      const post = held.get(edit.id)
      if (!post) {
        missing.push(edit.id)
        continue
      }
      if ((post.text ?? '') === edit.text) {
        unchanged.push(edit.id)
        continue
      }

      const channel = service.get(post.channelId)
      const cadence = channel?.cadence
      if (cadence?.maxLength && edit.text.length > cadence.maxLength) {
        throw new Error(
          `${channel.service} refuses a post past ${cadence.maxLength} characters, ` +
            `and ${edit.id} would be ${edit.text.length}`
        )
      }
      ...
      const input = {
        id: post.id,
        text: edit.text,
        assets: assetsAsInput(post.assets),
      ...
      if (dryRun) {
        rewritten.push({ id: post.id, status: 'not written', service: channel?.service })
        continue
      }
      const data = await gql(run, EDIT, { input })
      rewritten.push({ ...unwrap(data.editPost, 'rewrite'), service: channel?.service })
    }
  } catch (cause) {
    throw Object.assign(cause, { rewritten, missing, unchanged })
  }

  return { rewritten, missing, unchanged }
}

/**
 * Puts the posts Buffer could not publish back onto the free slots ahead of them.
 *
 * A failed post is the one thing in this queue nothing moves on its own. It
 * holds a day that has already gone, so no slot search competes with it and no
 * promotion reaches it, and it goes on being counted a failure on every reading
 * taken of the channel until somebody does something about it. Left alone it is
 * a day the account said nothing plus a permanent line in every report, and the
 * words on it were written, held to the voice and approved, so the day it missed
 * is the first thing to try rather than the bin.
 *
 * `editPost` replaces a post rather than patching it, the same difficulty
 * promoting has, so the assets are read back off the post and sent again. An
 * Instagram post requeued without its card is a second failure booked for a day
 * in the future, and a channel that publishes nothing without media on a post
 * that has none is stood down and named instead.
 *
 * @param {string|object} key A Buffer public API key, or a run from `connect`.
 * @param {object} [options]
 * @param {string[]} [options.ids] The posts to move. Every failed post when absent.
 * @param {Date} [options.now] The moment the slot search starts from.
 * @param {boolean} [options.dryRun] Read and choose the slots, and write nothing.
 * @returns {{requeued: object[], skipped: object[], missing: string[]}}
 */
export async function requeue(key, { ids = null, now = new Date(), dryRun = false } = {}) {
  const run = connect(key)
  const { organizationId, channels } = await wiring(run)
  const all = await posts(run, organizationId)
  ...
  const requeued = []
  const skipped = []
  // Named and not held is worth saying on its own. An id that has published or
  // been taken out reads exactly like one the run declined to move, and the
  // caller can do something about only one of those.
  const missing = wanted ? [...wanted].filter(id => !all.some(post => post.id === id)) : []

  try {
    for (const channel of channels) {
      const mine = all.filter(candidate => candidate.channelId === channel.id)
      const failed = mine.filter(
        candidate => isFailed(candidate) && (!wanted || wanted.has(candidate.id))
      )
      if (!failed.length) continue

      const scheduled = mine.filter(candidate => candidate.status === 'scheduled')
      const slots = freeSlots(channel.cadence, scheduled, failed.length, now)

      // The slot is taken when a post is actually placed on it rather than handed
      // out by position, so a post stood down does not take a day down with it.
      let next = 0
      for (const post of failed) {
        const assets = assetsAsInput(post.assets)
        if (channel.cadence.requiresImage && !assets.length) {
          skipped.push({
            id: post.id,
            service: channel.service,
            reason: `the post carries no image, which ${channel.service} will not publish`,
          })
          continue
        }
        if (next >= slots.length) {
          skipped.push({
            id: post.id,
            service: channel.service,
            reason: 'no free slot inside the horizon',
          })
          continue
        }
        ...
        const data = await gql(run, EDIT, { input })
        next += 1
        requeued.push({ ...unwrap(data.editPost, 'requeue'), service: channel.service })
      }
    }
  } catch (cause) {
    throw Object.assign(cause, { requeued, skipped, missing })
  }

  return { requeued, skipped, missing }
}

/**
 * Takes posts out of the queue, whatever state they are in.
 *
 * Where a failed post goes when requeueing it would publish the wrong thing: an
 * offer that has closed, a week that has passed, a caption whose card the
 * catalogue no longer serves. Nothing here decides that. The caller names the
 * ids, because a post in the queue is work somebody wrote and there is nothing
 * behind this that puts one back.
 *
 * @param {string|object} key A Buffer public API key, or a run from `connect`.
 * @param {string[]} ids The posts to take out.
 * @param {object} [options]
 * @param {boolean} [options.dryRun] Read and match, and write nothing.
 * @returns {{discarded: object[], missing: string[]}}
 */
export async function discard(key, ids, { dryRun = false } = {}) {
  const run = connect(key)
  const { organizationId, channels } = await wiring(run)
  const all = await posts(run, organizationId)
  const held = new Map(all.map(post => [post.id, post]))
  ...
  try {
    for (const id of ids) {
      const post = held.get(id)
      if (!post) {
        missing.push(id)
        continue
      }

      // Read off the post before it goes, because afterwards there is nothing
      // left to read and what was taken out is the whole of what a caller has.
      const taken = {
        id,
        service: service.get(post.channelId) ?? null,
        status: post.status,
        dueAt: post.dueAt ?? null,
        text: post.text ?? '',
      }
      if (dryRun) {
        discarded.push({ ...taken, deleted: false })
        continue
      }

      const data = await gql(run, DELETE, { input: { id } })
      if (data.deletePost.__typename !== 'DeletePostSuccess') {
        throw new Error(`discard: ${data.deletePost.__typename}: ${data.deletePost.message}`)
      }
      discarded.push({ ...taken, deleted: true })
    }
  } catch (cause) {
    throw Object.assign(cause, { discarded, missing })
  }
