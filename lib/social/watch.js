 ...
 */
import { CADENCE, RATE_LIMITED, connect, coversDays, posts, runwayDays, wiring } from './buffer.js'
import { dayIn } from '../time/zone.js'
import { misquotedPrices } from './voice.js'
...
export const FINDING = {
  ...
  roster: 'roster-stale',
  misquoted: 'post-misquotes-price',
  fault: 'watch-failed',
}
...
export const SEVERITY = {
  ...
  unknown: 'unknown',
  // Publishing has stopped, stops within days if nothing is done, or is about
  // to carry something false out of reach.
  alarm: 'alarm',
...
/** Statuses a post still has while there is time to change what it says. */
const PENDING = new Set(['scheduled', 'draft'])

/**
 * Posts on a channel that have yet to publish and state a price the site does
 * not.
 *
 * Read on the queue as well as at the point of writing, because the two catch
 * different things. `social.js post` refuses a stale figure before it is placed
 * and does nothing for the twenty already sitting there, or for one edited in
 * Buffer's own composer by a person no check runs for. Only what has yet to
 * publish: a sent post cannot be rewritten through Buffer, and a finding raised
 * every morning about something nobody can act on is how a report earns being
 * skipped.
 */
function priceFindings(channel, all) {
  const found = []
  for (const post of all) {
    if (post.channelId !== channel.id || !PENDING.has(post.status)) continue
    for (const stale of misquotedPrices(post.text)) {
      found.push(
        finding(FINDING.misquoted, SEVERITY.alarm, channel.service, {
          post: post.id,
          dueAt: post.dueAt ?? null,
          detail:
            `${post.dueAt ? `the ${channel.service} post due ${post.dueAt}` : `a ${channel.service} draft`} ` +
            `states ${stale.found} ${stale.term}, and the site publishes ${stale.published}. ` +
            'Rewrite it with social.js rewrite before it publishes, because a ' +
            'published post cannot be corrected through Buffer.',
        })
      )
    }
  }
...
export function judge({ channels, posts: all = [], now = new Date(), roster = EXPECTED }) {
  ...
  for (const service of Object.keys(CADENCE)) {
    ...
    findings.push(...priceFindings(channel, all))
