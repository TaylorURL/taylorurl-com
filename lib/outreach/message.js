...
import { CLIENT_REVIEWS as REVIEWS } from '../../src/app/data/reviews.js'
...
const REPLY_REACHES = 'A reply to this message comes straight to me, and I answer it myself.'
 ...
 * person typed: paragraphs in the client's own reading face, a name and a
 * number under them, the way off the list under that, and nothing else in the
 * envelope but the one square that says whether it was opened. No masthead,
 * no figure, no capture, no cards, no button. The two halves are built from
 * one content object the way the laid-out letter's are, so neither can say a
 * paragraph the other does not.
 */
...
/** The greeting, with the name where the address gave one and bare where it did not. */
const plainGreeting = name => (name ? `Hi ${name},` : 'Hi,')
...
export function renderPlainHtml(message) {
  ...
  const tracker = track
    ? `<img src="${esc(trackedImage(track))}" alt="" width="1" height="1" ` +
      'style="display:block;width:1px;height:1px;border:0;">'
    : ''
  // The sign-off, which is the whole of what the letter offers: a name, the
  // studio, the number it hopes to be rung on, and the studio's own address.
  // A cold introduction that gives no number is asking to be replied to, and a
  // reader who half means to answer does not open their mail client for it.
  const call = contact.phone
    ? `<br><a href="${esc(dial(contact.phone))}" style="color:${PLAIN_INK};">${esc(contact.phone)}</a>`
    : ''
  const site = contact.site
    ? `<br><a href="${esc(contact.site)}" style="color:${PLAIN_INK};">${esc(shownAs(contact.site))}</a>`
    : ''
...
<p style="margin:0 0 16px 0;${PLAIN_TYPE}">${esc(plainGreeting(message.greeting))}</p>
${message.paragraphs.map(plainParagraph).join('\n')}
<p style="margin:0;${PLAIN_TYPE}">${esc(SIGNATURE.name)}<br>${esc(SIGNATURE.studio)}${call}${site}</p>
${off}
</div>
${tracker}
</body>
...
export function renderPlainText(message) {
  const { contact } = message
  const lines = [plainGreeting(message.greeting), '']
