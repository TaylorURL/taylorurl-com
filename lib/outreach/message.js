 ...
 *
 * The message carries no figures for what a build costs, and there are none to
 * carry: every build is quoted for the project it is, once somebody has said
 * what theirs needs. Its job is a reply or a click, and /start is where a
 * reader who wants to look first says what they are after.
 *
...
import { escapeHtml as esc } from '../mail/escape.js'
...
/** The square a message with no capture carries instead: one pixel, with no alt text. */
const openSquare = track =>
...
// rather than approximated. The file is 260x64, twice its drawn size, so a
// retina screen has two device pixels per drawn pixel. Both single-ink forms
// travel, because the mark is used unaltered and a single ink only reads on the
// ground it was cut for: the black one for the light strip the work section sits
// on, the white one for a client that has drawn that strip dark. Naming the
// platform is the whole of what it is for, so neither form is recoloured to
...
// The field, from :root in src/index.css, written out flat because a mail
// client resolves no custom property. The paper is warm and the ink is warm
// charcoal, which is the palette the site is on.
const PAGE = '#f0eee6'
const SHEET = '#faf9f5'
const INK = '#141413'

// The three softer steps, which are the one part of the field that is not the
// alphas flattened onto the surface. The paper came down and the ink came up,
// so a flatten would land every step lighter than the neutral it replaces.
// They are cut instead to hold the contrast those neutrals held: #4c4a45 reads
// 8.40:1 on the sheet and 7.62:1 on the page where #4f4f4f read 8.19:1 and
// 7.57:1, #64625d reads 5.78:1 and 5.24:1 where #676767 read 5.66:1 and 5.23:1,
// and #706e68 reads 4.84:1 and 4.39:1 where #737373 read 4.74:1 and 4.38:1.
const INK_SOFT = '#4c4a45'
const INK_MUTE = '#64625d'
const INK_FAINT = '#706e68'

// The rules, which are the paper one step and two steps down from the sheet.
const HAIRLINE = '#e0ddd3'
const HAIRLINE_STRONG = '#cfcbbf'
const ACCENT = '#1a4ed8'
const ACCENT_FILL = '#2b64f0'
const ON_ACCENT = '#ffffff'

// The `data-ground="dark"` palette, which is the slab a light page sets into
// itself. The slab is warm charcoal rather than black and its ink is the page's
// own ivory, so lifting the ground costs everything drawn on it some contrast.
// Both steps on it are cut back for that: the accent takes the lighter of the
// two blues at 5.27:1 where #3d76ff read 5.23:1 on black, and the muted ivory
// reads 6.67:1 where #8c8c8c read 6.25:1.
const SLAB = '#1f1e1d'
const SLAB_INK = '#f0eee6'
const SLAB_INK_MUTE = '#a6a49c'
const SLAB_ACCENT = '#5c8cff'
...
export function renderHtml(message) {
  ...
  const tracker = track && !figure?.site?.shot ? openSquare(track) : ''
 ...
 */
// The letter's own two inks, which are the sheet's charcoal and a warm grey cut
// to hold what the neutral held: #706e68 reads 4.84:1 on the sheet where
// #737373 read 4.74:1 on white.
const PLAIN_INK = '#141413'
const PLAIN_MUTE = '#706e68'
...
export function partOfDay(at) {
  return clockIn(at).minutes < MIDDAY ? 'morning' : 'afternoon'
}
...
export function renderPlainHtml(message) {
  ...
  const track = message.track || null
  const tracker = track ? openSquare(track) : ''
