 ...
 * figures.
 *
 * The two ceilings bound different things and different things fix them. A
 * day's ceiling is what one mailbox can send as cold mail before the receiving
 * side starts reading the sender rather than the message. A run's ceiling is
 * what one invocation of the send job can deliver inside the function's own
 * time limit. The day is spread across many runs by lib/outreach/schedule.js,
 * so the second is not a division of the first and neither moves when the other
 * does.
 *
 ...
 * lib/outreach/ramp.js decides with them and the console draws the same
 * thresholds it does.
 */

/**
 * Messages one day of outreach can carry.
 *
 * Sending is over Gmail SMTP as one Google Workspace mailbox, so this is the
 * whole of the day and it is a statement about reputation: past it the volume
 * itself is what the receiving side reads. A hundred and fifty is set by hand
 * above the plateau a single mailbox holds cleanly, so the working cap can be
 * pushed for volume; the trailing bounce rate is what says whether it held,
 * and the ramp rolls the cap back to the floor and halts once that rate
 * crosses its threshold. The settings column refuses a larger figure and the
 * console's field offers this as its maximum.
 */
export const DAILY_CAP_MAX = 150
 ...
 *
 * DAILY_CAP_MAX is a statement about reputation, and reputation is carried far
 * more by complaints than by bounces: a hard bounce says an address does not
 * exist, which is a fact about the list, while a complaint says a person who
 * read the message reported it, which is the fact the plateau is about. Gmail
 * SMTP runs no feedback loop, so nothing here can read a complaint, and
 * outreach_messages has no column that could hold one. Google Postmaster Tools
 * is the only route to that signal and it is not set up.
 *
 ...
 * introduction away from joining. Held at the day's cap, a day can always send
 * a full day, and the pipeline behind it has a day of slack to find the next
 * hundred and fifty. Below it the sourcing and the enrichment are not keeping
 * up with the sending, which is the one thing that stops the rotation growing.
 *
 * The same figure as DAILY_CAP_MAX, and the same figure for a reason: a floor
 * under the queue that was lower than the day's cap would be a floor a single
 * day could sweep straight through.
 */
export const QUEUE_FLOOR = DAILY_CAP_MAX
