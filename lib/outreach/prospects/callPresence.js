 ...
 * off it new as well - and one of those is the list of columns the table is
 * drawn from, so the whole table, every cell of it, was being rebuilt three
...
/** A caller's initials, for where there is room for two letters and no more. */
export function callerMark(row) {
...
/** The same run of time, said as how long ago it was rather than how long it is. */
export function saidAgo(value, now = new Date()) {
  const said = saidSince(value, now)
  return said === 'just now' ? said : `${said} ago`
}
