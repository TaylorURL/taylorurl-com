...
import { ASSIGNED_MINE, ASSIGNED_NOBODY } from './calls.js'
import { DEFAULT_GOALS, goalsPatch, normalizeGoals } from './callShift.js'
 ...
 *
 * `align` is which edge the cell sets its content against, where that is not
 * the reading edge. `end` is the far edge, and only the control column asks
 * for it: a button is set against the card's own right gutter rather than
 * left in the middle of a column that is wider than it is.
 */
export const LIST_COLUMNS = Object.freeze([
  Object.freeze({ id: 'business', label: 'Business', weight: 20, fixed: true }),
  ...
  Object.freeze({ id: 'assigned', label: 'Assigned To', weight: 11, at: 'lg' }),
  Object.freeze({ id: 'state', label: 'State', weight: 12, fixed: true }),
  // The one column whose content has a floor under it. Every other cell is
  // text and gives way by truncating; this one holds a control a thumb has to
  // land on, so a share that leaves it thirteen pixels of content box does not
  // draw a narrower button - it draws the same button, over the cell beside it
  // and past the edge of the card. Twelve is what keeps it inside its own cell
  // at a phone's width, where the drawn set is five columns and the shares are
  // at their tightest.
  Object.freeze({ id: 'call', label: 'Call', weight: 12, fixed: true, align: 'end' }),
])
 ...
 * is narrowed.
 */
export function filterChips(filters, people = []) {
  const held = normalizeFilters(filters)
  const chips = []
  for (const [key, options] of Object.entries(LISTED_FILTERS)) {
  ...
  if (held.assigned !== 'all') {
    chips.push({ key: 'assigned', label: assignedLabel(held.assigned, people) })
  }
  return chips
}

/**
 * What a filter on who holds a business is called, on a chip.
 *
 * The two standings say themselves. An id has to be looked up, and where the
 * lookup comes back empty the id is not printed at somebody: a chip reading
 * `9f3a...` says less than the word does and cannot be acted on either.
 */
export function assignedLabel(value, people = []) {
  if (value === ASSIGNED_MINE) return 'Mine'
  if (value === ASSIGNED_NOBODY) return 'Nobody Yet'
  const found = (Array.isArray(people) ? people : []).find(one => one?.id === value)
  return found?.name || 'One Person'
}
