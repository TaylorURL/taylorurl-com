...
export const HANDBOOK_PARTS = Object.freeze([
  { id: 'open', label: 'Opening The Call' },
  { id: 'questions', label: 'Questions They Ask' },
  { id: 'pushback', label: 'When They Push Back' },
  { id: 'facts', label: 'The Facts' },
  { id: 'close', label: 'Closing The Call' },
])
...
function listingLine(row) {
  ...
  if (presence === 'none') {
    return (
      'I looked you up on Google and your listing has the phone number and the map pin, ' +
      'but there is no website on it at all. So anybody who wants to know your hours or ' +
      'your prices has to call you to find out.'
    )
  }
  if (presence === 'portal') {
    return (
      `I looked you up on Google and the website on your listing goes to ${platform || 'a directory'}, ` +
      'which is a directory somebody else put you in. It has your name on it and their phone ' +
      'number and their advertising around it.'
    )
  }
  if (presence === 'booking') {
    return (
      `I looked you up on Google and the website on your listing is your ${platform || 'booking'} page. ` +
      'You are paying for that page every month and it is theirs — their address, their branding ' +
      'at the bottom, and nothing on it that Google can rank as yours.'
    )
  }
  return (
    `I looked you up on Google and the website on your listing is your ${platform || 'social'} page. ` +
    'That is somebody else’s platform. They decide who sees it, and if they close the account ' +
    'tomorrow there is nothing of yours left online.'
  )
}
...
function tradeLine(row) {
  ...
  if (typeof count !== 'number' || median === null || median === undefined) {
    return (
      'You have been doing this a while and the work is clearly there. What is missing is ' +
      'somewhere for the people looking for it to land.'
    )
  }
  const middle = Math.round(median)
  if (row?.pull === 'busy') {
    return (
      `You have ${count} reviews. The middle for ${trade} listings around here is about ${middle}, ` +
      'so you ' +
      'are well ahead of the shops you are competing with — and every one of them that has a ' +
      'website is getting found ahead of you anyway.'
    )
  }
  if (row?.pull === 'quiet') {
    return (
      ...
      `${middle}. That ` +
      'is not a reflection of the work — it is what happens when there is nowhere for people to ' +
      'find you and nothing asking them to leave one.'
    )
  }
  return (
    `You have ${count} reviews, which is about where most ${trade} listings sit around here. The ` +
    'shops pulling ahead of that are the ones people can find on a phone at eight at night.'
  )
}
...
function proofLine(row) {
  ...
  if (row?.proof === 'trade' && named) {
    return (
      `We have built for your line of work already — ${named}. Look it up while we are talking; ` +
      'it is the same job as yours.'
    )
  }
  if (row?.proof === 'town' && named) {
    return `We built ${named}, which is right there by you. Have a look at it while we are talking.`
  }
  return (
    'We are a small team in Baytown and everything we have built is on our site with the client ' +
    `names on it. Google’s own speed test averages ${PORTFOLIO_AVERAGES.mobile} on mobile across ` +
    'those sites, which you can run yourself on any of them.'
  )
}

/** The ask. One sentence, and it asks for a conversation rather than a sale. */
const ASK =
  'What I would like to do is put together a plan for what your site would say and what it ' +
  'would cost. That part is free, and you see the number before anything starts. Have you got ' +
 ...
 */
export function scriptFor(row) {
  const held = (row?.calls ?? []).some(call => call?.outcome === 'gatekeeper')
  return [
    {
      id: 'greeting',
      label: 'Get To The Owner',
      say: held ? GREETING_AGAIN : GREETING,
    },
    {
      id: 'opener',
      label: 'Who Is Calling',
      // The caller's own name is the one word in the handbook nobody else can
      // write. A bracket is what a script has always used for that, and it is
      // read as a slot rather than as a line to say.
      say:
        `This is [your name] with ${SITE.shortName}. We build websites for businesses around ` +
        'Baytown and Houston. I am not selling anything over the phone — I called about ' +
        'something on your Google listing.',
    },
    { id: 'reason', label: 'Why You Called', say: listingLine(row) },
    { id: 'trade', label: 'What Their Trade Says', say: tradeLine(row) },
    { id: 'proof', label: 'Work To Name', say: proofLine(row) },
...
export const QUESTIONS = Object.freeze([
  {
    ...
    say:
      `${BUILD_PRICE} up front for the build and ${MONTHLY_PRICE} a month after that. The monthly ` +
      'covers the hosting, the backups, the security, any changes you want, and the search work ' +
      'every month. Most sites land right there. If yours needs more than that you see the number ' +
  ...
  },
  {
    ...
    say:
      'Hosting, daily backups, security updates, and the search work every month. Practically: ' +
      'it stays online, it stays backed up, you text me when a price changes and I change it ' +
      'the same day, and somebody is working on getting you found the whole time.',
  },
  {
    id: 'contract',
    ask: 'Am I locked into anything?',
    say:
      'No. There is no term to sign and no cancellation fee. You can stop the monthly whenever ' +
      'you want — the site runs to the end of what you have paid for and comes down after that. ' +
      'The domain is in your name either way.',
  },
  {
    id: 'how-long',
    ask: 'How long does it take?',
    say:
      'Most sites are live in two to four weeks. What usually slows it down is photos, so if you ' +
      'have any of the work already on your phone that is most of it.',
  },
  {
    id: 'what-i-do',
    ...
    say:
      ...
      'have. We write the words and design the thing. If nothing is ready we build it as we go ' +
  ...
  },
  {
    ...
    ask: 'Do I own it?',
    say:
      'The domain is registered in your name, and your words, your photos and your logo stay ' +
      'yours. We keep the code and the platform it runs on, and that is written into the ' +
      'agreement so nobody is guessing later.',
  },
  {
    id: 'changes',
    ask: 'What if I need to change something?',
    say:
      'Text me. Text and photo changes are part of the monthly and usually go live the same day. ' +
      'There is no ticket and no change fee. A whole new page or a new feature gets a price first.',
  },
  {
    id: 'wordpress',
  ...
  },
  {
    ...
    say:
      ...
      'payments, quote forms, customer logins — whichever of those your shop actually runs on.',
  },
  {
    id: 'google',
    ...
    say:
      ...
      'month after, aimed at what people near you actually type. Your Google Business Profile ' +
      'gets set up properly at the same time.',
  },
  {
    id: 'who',
    ask: 'Who am I actually dealing with?',
    say:
      'Me, and the same small team the whole way through. There is no account manager and no ' +
      'queue. A year from now you are still calling this number.',
  },
  {
    id: 'where',
    ask: 'Where are you?',
...
export const PUSHBACK = Object.freeze([
  {
    ...
    say:
      'Right, and that is doing something for you — that is exactly why this is worth ten ' +
      'minutes. A Facebook page shows people who already follow you. It does not come up when ' +
      'somebody types your trade and your town into Google at eight at night, and you cannot ' +
      'take it with you if they lock the account.',
  ...
  },
  {
    ...
    say:
      `Fair enough. For what it is worth, the same build gets quoted between ${dollars(MARKET.buildLow)} ` +
      `and ${dollars(MARKET.buildHigh)} by an agency, and the monthly they put on top is usually ` +
      `more than ${MONTHLY_PRICE}. But the number is not the question yet — the question is ` +
      'whether one extra job a month would cover it. If it would not, I am the wrong call.',
    after:
  ...
  },
  {
    ...
    say:
      'That is the answer I hear from the shops worth calling. It is half an hour of yours in ' +
      'total, and it is half an hour whenever suits — early, late, or I come to the shop. What is ' +
      'the quietest part of your week?',
  ...
  },
  {
    ...
    say:
      'I will, gladly. What address is best? And so I send you something worth reading rather ' +
      'than a brochure — what is the one thing you would want a website to actually do for you?',
    after:
  ...
  },
  {
    ...
    say:
      'Good — genuinely, that is better than nothing. The part that usually goes wrong is not the ' +
      'building, it is the year after: who updates it, who is watching when it goes down, who ' +
      'does the Google side every month. If that lands back on you, call me.',
  ...
  },
  {
    ...
    say:
      'That is the best kind of work there is. Here is the thing though — somebody who gets your ' +
      'name from a friend still types it into their phone before they call you. What they find ' +
      'right now is a map pin. That is a referral you already earned, deciding whether to ring you.',
  ...
  },
  {
    ...
    say:
      'No problem at all. When is the best time to catch them, and is this the right number? I am ' +
      'not selling over the phone — it is about their Google listing.',
...
export const CLOSING = Object.freeze([
  {
    id: 'collect',
    label: 'Get These Before You Hang Up',
    say:
      'Their name, the best email, and the best number. Then: what the site has to do — take ' +
      'bookings, take orders, get the phone ringing, or just exist properly.',
  },
  {
    id: 'promise',
    label: 'Say What Happens Next',
    say:
      'You will have a written plan and a price by email, and nothing is charged to get it. ' +
      'Read it, and if the number works we start.',
  },
  {
    id: 'record',
    label: 'Record It Right',
    say:
      'Booked The Work takes them off the list. Anything short of a yes with a date on it is a ' +
      'call back with the time they gave you, not a booking.',
  },
  {
    id: 'send',
    label: 'Then, Off The Phone',
...
export function handbookMatches(entry, typed) {
  ...
  const hay = [entry?.ask, entry?.said, entry?.say, entry?.label, entry?.note, entry?.after]
    .filter(Boolean)
    .join(' ')
    .toLowerCase()
