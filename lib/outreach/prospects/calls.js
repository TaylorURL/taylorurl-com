 ...
 * business nobody can find, which is the entire thing being sold. It scores
 ...
 * for one where it is set and the endpoint refuses the write without it,
 * because a callback with no time on it is a promise nothing will ever surface
 * again.
 *
 ...
 * outcome cannot answer and the caller has to, because Spoke To Owner covers
 * both halves of the only conversation that matters: the owner who asked what
 * it would cost and the owner who said no thanks and hung up. An outcome
 * carrying neither reached nobody who could have wanted anything, so there is
 ...
 * by the check that holds it. A cold prospect is not a lead: eight thousand
 * names off a map are a list to work, and a section that files every
 * conversation as a lead buries the handful who actually asked for something
 * under the ones who said no thanks and hung up.
 */
export function callMakesLead(outcome, answered = null) {
  return interestIn(outcome, answered) === true
}
...
export const PLACES = Object.freeze([
  ...
  { id: 'promised', label: 'Ringing Back', tone: 'plain', calls: false },
 ...
 * rather than sloppy: a business that is busy in its trade, in a trade the
 * studio has work in, on a booking platform, and has already been spoken to is
 * off the top of any scale worth drawing, and clamping says so more honestly
 * than rescaling every other row to make room for it.
 *
...
export const SCORE_WEIGHTS = Object.freeze({
  trade: Object.freeze({ busy: 50, steady: 22, quiet: 16, unread: 12 }),
  proof: Object.freeze({ trade: 20, town: 10, none: 0 }),
  presence: Object.freeze({ booking: 25, social: 15, none: 10, portal: 0 }),
 ...
 *
 * Reviews first, because a hundred of them is more trade than nine however the
 * bands read; then the oldest listing, which has been waiting longest; then
 * the id, so the order is total and a page boundary cannot show one business
 * twice or none.
 */
export function byCallOrder(now = new Date()) {
  return (one, two) => {
    ...
    if (rank) return rank
    const counts = (two.rating_count ?? -1) - (one.rating_count ?? -1)
