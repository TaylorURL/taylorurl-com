 ...
 *
 * What a first call is for changed under all of it and the shape held. The
 * caller no longer pitches or quotes: the one goal is a yes to a free audit and
 * a time the same day to go through it, which is `audit_booked` below. So two
 * outcomes now carry a time rather than one, and every rule that used to read
 * the word `callback` reads `outcomeTakesCallback` instead - a rule keyed on the
 * one id would have filed the new outcome as a promise with nothing to surface
 * it, which is the failure the whole timing section is built around.
 *
...
import { BOOKING_HOSTS, PORTAL_HOSTS, hostOf, platformOf } from './platforms.js'
 ...
 * `none`.
 *
 * For the pipeline every platform host answers one question the same way - the
 * listing names no site of the business's own. On the phone they are not the
 * same at all, and the difference is the opening sentence.
 *
 ...
 * altogether. What an outcome that keeps the business cannot carry is no time
 * at all - that is a promise nothing will ever surface again - so one nobody
 * timed is filed at the default its own outcome names.
 *
 ...
 * itself, and four of them can - a booking, an audit booked and a named time to
 * ring back are a yes, and Not Interested is the word itself. `asks` marks the
 * two where the outcome cannot answer and the caller is offered the question,
 * because Spoke To Owner covers both halves of the only conversation that
 * matters: the owner who asked what it would cost and the owner who said no
 * thanks and hung up. An outcome carrying neither reached nobody who could have
 * wanted anything, so there is nothing to ask and nothing to record.
 *
 * The keys are the order the tiles are drawn in and a caller presses them
 * without looking, so they are single digits and they stay put. Ten outcomes
 * against nine digits is what `bad_lead` takes the nought for: it is the odd
 * one out already, filed off its own control rather than about the
 * conversation, and it is the tile a caller reaches for least.
 */
export const CALL_OUTCOMES = Object.freeze([
  { id: 'no_answer', label: 'No Answer', tone: 'plain', key: '1' },
  { id: 'voicemail', label: 'Left Voicemail', tone: 'plain', key: '2' },
  { id: 'gatekeeper', label: 'Gatekeeper', tone: 'plain', asks: true, key: '3' },
  { id: 'spoke', label: 'Spoke To Owner', tone: 'accent', asks: true, key: '4' },
  { id: 'callback', label: 'Call Back', tone: 'accent', keeps: true, interest: true, key: '5' },
  // The one goal of a first call: the owner said yes to a free audit and named
  // a time the same day to be walked through it. A different fact from a plain
  // call back, which is somebody agreeing to hear from the caller again and
  // nothing more - this one has the audit team building something before the
  // phone rings again, and the callback it carries is hours out rather than a
  // day.
  {
    id: 'audit_booked',
    label: 'Audit Booked',
  ...
  { id: 'booked', label: 'Booked', tone: 'good', ends: true, interest: true, key: '7' },
  {
    id: 'not_interested',
  ...
  { id: 'wrong_number', label: 'Wrong Number', tone: 'bad', ends: true, key: '9' },
  ...
  { id: 'bad_lead', label: 'Bad Lead', tone: 'bad', ends: true, key: '0' },
])
 ...
 *
 * Neither outcome that carries a time is here, and that absence is
 * load-bearing. A named time is a promise rather than a rest, and a rest that
 * competed with it would swallow it: a caller told at ten in the morning to
 * ring back at four would find the business resting until tomorrow, which is
 * the list overruling the only person on it who knows anything.
 */
...
/**
 * How far out an audit that nobody timed lands, which is the same afternoon.
 *
 * A day is the wrong answer for this one and it is wrong in a way that costs
 * the call: the owner agreed to look at an audit of their own listing, and the
 * thing that gets it looked at is ringing back while they still remember
 * agreeing. Four hours reaches the other half of a working day, so a morning
 * call comes back after lunch and an afternoon call comes back before closing.
 */
export const AUDIT_CALLBACK_DEFAULT_HOURS = 4
...
/**
 * Where an untimed promise lands, from now or from a moment handed in.
 *
 * The outcome decides the length, because the two outcomes that carry a time
 * are promises about different things. A call back is somebody agreeing to hear
 * from the caller again, and a day is the shortest gap that reaches a different
 * shift. An audit is a thing being built for them today, so its default is the
 * same day.
 *
 * @param {Date} [now]
 * @param {string} [outcome] What the call came to.
 */
export function defaultCallbackAt(now = new Date(), outcome = 'callback') {
  const hours = outcome === 'audit_booked' ? AUDIT_CALLBACK_DEFAULT_HOURS : CALLBACK_DEFAULT_HOURS
  return callbackIn(hours, now)
}
...
/**
 * The lengths an audit is walked through in, which are all inside the day.
 *
 * The ladder starts at tomorrow, and tomorrow is the one thing a booked audit
 * must not be: the owner said yes to looking at something this afternoon, and a
 * screen whose shortest press is the next day hands the caller a choice between
 * a time that is wrong and typing nothing at all. So these sit in front of the
 * ladder rather than replacing it, because the owner who can only manage
 * Thursday is still an audit booked.
 *
 * The labels are how a caller says the length to somebody rather than how it is
 * counted, and they are pressed with a phone against an ear, so each is short
 * enough to read off a tile at a glance.
 */
export const SAME_DAY_LENGTHS = Object.freeze(
  [
    { hours: 1, label: 'In an Hour' },
    { hours: 2, label: 'In Two Hours' },
    { hours: 4, label: 'This Afternoon' },
  ].map(Object.freeze)
)

/**
 * The lengths the screen offers for an outcome that carries a time.
 *
 * Asked of the outcome rather than drawn as one fixed set, because the two
 * promises are made in different units: an audit is gone through today and a
 * call back is somewhere in the next fortnight.
 *
 * @param {string} outcome What the call came to.
 * @returns {ReadonlyArray<{hours: number, label: string}>}
 */
export function callbackLengthsFor(outcome) {
  return outcome === 'audit_booked'
    ? Object.freeze([...SAME_DAY_LENGTHS, ...CALLBACK_LENGTHS])
    : CALLBACK_LENGTHS
}
 ...
 * times must not land on the thirty-day rung. `spoke` and both outcomes that
 * name a time reset the run to nothing, because reaching a person is the
 * opposite of the evidence this counts.
 *
 ...
 * doubles as "this outcome carries its own time". An outcome that carries a
 * time returns null too, because what decides its return is the time somebody
 * named rather than any number here.
 */
export function waitHoursFor(outcome, run) {
  if (!outcomeOf(outcome) || outcomeEnds(outcome) || outcomeTakesCallback(outcome)) return null
...
export function readyAt(row, now = new Date()) {
  ...
  const hours = outcomeTakesCallback(last.outcome)
...
/** Whether a business belongs to somebody other than the caller reading the list. */
export function heldByOther(row, userId = null) {
  const owner = ownerOf(row)
  return owner !== null && owner !== userId
}

/**
 * An order with every colleague's business moved behind the caller's own.
 *
 * Each caller works their own queue: what they hold and what nobody holds yet.
 * A business somebody else rang first is still on the list, because a colleague
 * out for the day should not strand it, but it is offered only once there is
 * nothing of the caller's own left above it - a colleague's callback coming due
 * included, since that promise was made by somebody else.
 *
 * @param {string|null} userId
 * @param {(one: object, two: object) => number} compare The order within each half.
 */
export function othersLast(userId, compare) {
  return (one, two) => {
    const theirs = Number(heldByOther(one, userId)) - Number(heldByOther(two, userId))
    return theirs || compare(one, two)
  }
}
 ...
 *
 * All of that runs inside the caller's own half of the list. Handed the reading
 * caller's id, every business a colleague holds sorts behind it - see
 * `othersLast`.
 */
export function byCallOrder(now = new Date(), userId = null) {
  return othersLast(userId, (one, two) => {
