 ...
 */
const REACHED_OUTCOMES = Object.freeze([
  'spoke',
  'callback',
  'audit_booked',
  'booked',
 ...
 *
 * Exported because the team's figures are the same question asked over more
 * than one day and more than one person, and a second list of outcomes kept
 * next to that arithmetic is how the middle figure comes to mean one thing on
 * the caller's own day and another on the board above it.
 */
export function reachedOwner(outcome) {
...
export function countsOf(today) {
  ...
  return {
    placed: rows.length,
    reached: rows.filter(call => reachedOwner(call?.outcome)).length,
