...
import { GOOD_FLOOR, bandOf, supporting, verdict, verify } from '../audit/bands.js'
...
/**
 * A category score as the figure a letter turns on: the score, the band, the
 * page it was read on and where to run it again, shown the same way whichever
 * category the letter quotes. What the score means and what else the report
 * marked short are the letter's own.
 *
 * @param {object} prospect A candidate row.
 * @param {string|null} shot The stored capture of the page, or nothing.
 * @param {{label: string, score: number|null, meaning: string, also: object[]}} reading
 */
export function scoreFigure(prospect, shot, { label, score, meaning, also }) {
  return {
    label,
    meta: 'Mobile',
...
export function speedFigure(prospect, shot) {
  const score = scoreOf(prospect)
  return scoreFigure(prospect, shot, {
    label: 'PageSpeed',
    score,
...
// offers one.
const SHORTFALL = where => [
  `At that score, a person who finds you on a phone waits on a blank screen, and some of them leave before the page opens. Google ranks a slow page lower for the same reason. It's fixable, and it's most of what we do for small businesses ${where}.`,
...
const NOTHING_SHORT = where => [
  `Nothing in that reading falls short, so there's nothing here for us to sell you. We build and look after sites for small businesses ${where}, and most of what reaches us is a site that stopped keeping up with the business behind it.`,
...
export function speedOpener(prospect, where, shot) {
  ...
  return {
    ...
    lines: [
      score === null
        ? `We ran ${name}'s website through Google's PageSpeed test this week, the way most of your customers arrive: on a phone, on a phone connection.`
