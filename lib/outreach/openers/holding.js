...
const BEHIND = where => [
  `It has slipped. That's fixable, and it's most of what I do for small businesses ${where}: sites that were fine when they went up and haven't kept pace with the business behind them.`,
  OFFER('the list of what is slowing yours down, in order'),
]

// What follows a reading with nothing short in it.
const HOLDING = where => [
  `It's holding up. Nothing in that reading falls short, so there's nothing here for me to sell you. If there's something the site isn't doing for the business yet, that's the conversation worth having, and it's most of what I do for small businesses ${where}.`,
...
export function holdingOpener(prospect, where, shot) {
  ...
  return {
    ...
    lines: [
      ...
      `I ran ${name}'s site through Google's PageSpeed test this week to see how it's holding up, the way most of your customers arrive: on a phone, on a phone connection.${scoreOf(prospect) === null ? '' : ` ${CHECK_LINE}`}`,
    ],
