...
export function plainSearchOpener(prospect, where, shot, context = {}) {
  ...
  if (searchShort(prospect)) {
    return plainLetter(subject, [
      ...
      `Want me to send what it marked on yours? ${YES}`,
      workLine({ site: context.site }),
    ])
  }

  return plainLetter(subject, [
    `One more from me on the site, then I'll leave it with you.`,
    `The other thing I look at is what a phone visitor can do in the first five seconds: call you, see your hours, ask for a quote. Most sites make them hunt for all three.`,
    `If you tell me what people mostly want from you when they land there, I'll tell you what I'd move. One line is plenty.`,
    workLine({ site: context.site }),
  ])
}
