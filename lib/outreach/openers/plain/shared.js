 ...
 */
export const HELD_BACK = "That's what I'd have opened with, if I opened with problems."
...
export function loadLine(prospect) {
  ...
  if (seconds !== null) {
    return (
      `I loaded ${host} on a phone connection this week. It was ${seconds} seconds before ` +
      `the biggest thing on the page showed up. ${GIVES_UP_AT}`
    )
  }
  if (typeof prospect.audit_score === 'number') {
    return (
      `I ran ${host} through Google's speed test this week, on a phone connection. ` +
      `It puts the site at ${prospect.audit_score} out of 100, and the wait is what that number is about. ${GIVES_UP_AT}`
    )
  }
  return `I tried to load ${host} on a phone connection this week, and Google's test could not put a number on it, which is usually a sign of its own.`
}
