...
/**
 * The sentence after the load time: what causes most of the wait where the
 * report names it, and that it is a short job either way.
 */
export function causesLine(prospect) {
...
/**
 * The finding in a letter to a business whose listing points at a platform:
 * where the listing sends a searcher, and what the searcher lands on.
 */
export function listingLines(prospect) {
...
export function workLine(context) {
  ...
  return `${named} is one of ours. The rest are here: ${site}`
}
