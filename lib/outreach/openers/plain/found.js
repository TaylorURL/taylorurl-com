...
export function foundOpener(prospect, where, shot, context = {}) {
  ...
  if (isClean(prospect)) {
    return plainLetter(subject, [
      `I loaded ${host} on a phone connection this week and it came up fast. That's rarer than it sounds, so there's nothing for me to fix there.`,
      `If there's something the site isn't doing for the business yet, a page it needs or a form nobody fills in, that's the thing I'd be worth a note on.`,
  ...
  return plainLetter(subject, [
    loadLine(prospect),
    causes,
    `${HELD_BACK} Want the list of what I'd change, in order, with what each takes? ${YES}`,
  ])
}
