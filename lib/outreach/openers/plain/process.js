...
export function processOpener(prospect, where, shot, context = {}) {
  return plainLetter(threaded(context, 'how a build goes'), [
    `Following our last note, here's how one actually runs, since it's the part nobody explains up front.`,
    `We talk once about what the site has to do. We build the first version and send you a link to it. You tell us what's wrong. We go round until it's right, it goes live, and then we look after it.`,
    `Your side of that is two conversations and a few links. There is no login you'll never open and no monthly bill for work nobody did.`,
