...
import { CATEGORIES, GOOD_FLOOR, bandOf, verify } from '../bands.js'
...
export function lowestOpener(prospect, where, shot, context = {}) {
  ...
  return {
    ...
    marker: '// Second Look',
    lines: [
      `One thing off the report I sent you a few days ago, in case the whole thing was more than you wanted to read.`,
    ],
    figure: mark
      ? {
          label: 'Lowest Mark',
          ...
          band: bandOf(value),
          meaning: short
            ? `${mark.label} is the lowest of the four marks and usually the quickest to bring up, so it's where I'd start.`
            : `${mark.label} is the lowest of the four marks, and it still sits in the band Google calls good. There's nothing on that report I'd open with.`,
    ...
    after: short
      ? [
          `That's usually a short job rather than a rebuild, and it's the kind of work I do for small businesses ${where}.`,
          OFFER('the list of what I would change, in order'),
        ]
      : [
