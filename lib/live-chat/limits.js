...
export function overCeiling(counts) {
  if (counts.sessionTurns >= LIMITS.perSessionTurns) {
    return 'That is a long thread, and I have reached the end of what I can carry in one. The team picks it up from here: trenton@taylorurl.com, or (281) 862-8687.'
  }

  if (counts.callerHour >= LIMITS.perCallerHour || counts.callerDay >= LIMITS.perCallerDay) {
    return 'That is as much as this connection gets for now. If there is more to talk about, we are at trenton@taylorurl.com or (281) 862-8687.'
  }

  if (counts.day >= LIMITS.perDay) {
    return 'The assistant is at its limit for today. We read everything sent to trenton@taylorurl.com, and answer.'
  }
