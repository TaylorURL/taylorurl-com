 ...
 * landed in. A design read that took only what a table holds would say the
...
import { auditEmail } from './audit.js'
...
import { renderIssueEmail } from './emailTemplate.js'
import { clientNotice, readNotification } from './notify.js'
import { capturedIcon, iconGround } from '../../src/app/data/console/siteIcons.js'
...
// The same business again, carrying the instant the reading was taken and the
// address the report settled on. The cold letters quote seconds and causes and
// never a date, so those two columns are only read by the audit message, and
// the sample it draws from has to hold them or the reading it renders is the
// one a row with no report gets.
const PROSPECT_AUDITED = {
  ...PROSPECT_MEASURED,
  audit_at: '2026-09-12T13:20:00Z',
  audit_raw: { ...PROSPECT_MEASURED.audit_raw, final_url: PROSPECT_MEASURED.website },
}

// The call the caller booked before pressing send, which the audit message
// closes on. A Monday afternoon, because the preview is read for how the time
// is said and a weekday name is the part most likely to be said wrong.
const NEXT_CALL = '2026-09-14T21:00:00Z'
...
// figure opens at the floor, which is what a link written without an override
// charges.
 ...
 */
/**
 * A lead as a paid ad delivers one: a name, an address, a phone, and the form
 * and campaign that produced it. The notice about a lead is drawn from this
 * row, so the sample carries every field the message prints.
 */
const LEAD = {
  name: 'Dana Whitfield',
...
export const FAMILIES = [
  ...
  {
    slug: 'audit',
    label: 'Site audit, sent off a call',
    system: 'outreach',
    from: 'TaylorURL <audits@taylorurl.com>',
    keeps: true,
    note: 'Sent by the caller on the phone, once the owner has agreed to see the reading in writing. Carries the four scores, what the report found and the date it was taken, and no figure of any kind: the price is worked out on the call that follows. The send is recorded against the prospect.',
  },
  {
    slug: 'lead-notice',
    ...
    from: 'TaylorURL Website <website@taylorurl.com>',
    keeps: false,
    note: 'Sent to the studio when a paid ad delivers a lead, carrying the form and the campaign that produced it. The lead is kept; the notice is not.',
  },
  {
    slug: 'confirmation',
...
export function renderFamily(slug, { track = null } = {}) {
  ...
  if (slug === 'audit') {
    // Rendered by the same function the call desk sends through, off the
    // measured business the plain letters are drawn for, so one business runs
    // through the cold letter, the reading and this.
    const drawn = auditEmail({
      prospect: PROSPECT_AUDITED,
      nextCall: NEXT_CALL,
    })
    return { subject: drawn.subject, html: drawn.html, text: drawn.text }
  }
  if (slug === 'lead-notice') {
    ...
    const drawn = notice({
      label: 'Paid ad',
      subject: `${LEAD.name} asked for a quote`,
      rows: [
        ['Name', LEAD.name],
        ['Address', LEAD.email],
        ['Phone', LEAD.phone],
