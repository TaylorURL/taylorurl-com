 ...
 * scores, the same issues and the same links as one showing the laid-out part.
 *
...
/**
 * The one thing the message asks for, and the sentence that says why.
 *
 * The address is drawn twice on purpose: as the button, and as itself
 * underneath, because a client that will not render a table-and-anchor button
 * still renders the line.
 */
function callToAction(startUrl) {
  const href = escapeHtml(startUrl)
  return (
    `<p style="margin:0 0 18px 0;${sans({ size: 15, color: INK_SOFT })};">The next step is the call you booked. We read this report through with you on it, and we price the work off what the site has to do.</p>` +
    '<table role="presentation" border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">' +
    `<tr><td align="center" bgcolor="${ACCENT}" style="border-radius:4px;background-color:${ACCENT};">` +
    `<a href="${href}" style="display:inline-block;padding:14px 26px;${sans({ size: 15, weight: 500, height: 1.2, color: '#ffffff' })};text-decoration:none;">Start a Project</a>` +
    '</td></tr></table>' +
    `<p style="margin:14px 0 0 0;${sans({ size: 13, color: INK_FAINT })};word-break:break-all;"><a href="${href}" style="color:${ACCENT};text-decoration:underline;">${href}</a></p>`
  )
}
 ...
 * the same date, the same two links and the same way off the list.
 */
function auditText({ name, site, scores, issues, at, startUrl }) {
  const when = formatInstant(at, { month: 'long', day: 'numeric', year: 'numeric' }, '')
  ...
  lines.push(
    '',
    'The next step is the call you booked. We read this report through with you on it, and we price the work off what the site has to do.',
    '',
    `Start a project: ${startUrl}`,
    '',
    '--',
    TRADING_LINE,
    `Or ring ${BIO_PHONE}`
 ...
 * @param {string} options.startUrl Where the one button goes.
 ...
 */
export function auditEmail({ prospect, startUrl, now = new Date() }) {
  ...
  const content = [
    ...
    band(callToAction(startUrl), '28px 40px 0 40px'),
