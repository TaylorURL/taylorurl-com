...
/** The paper a message stands on, and the sheet it is drawn on. */
export const PAGE = '#f0eee6'
const CARD = '#faf9f5'

/** The ink, which is warm charcoal rather than near-black. */
export const INK = '#141413'

/**
 * The two softer steps, which are warm greys cut a shade darker than the
 * neutrals they replace so the warmer paper costs no contrast. #44423d reads
 * 9.53:1 on the card and 8.64:1 on the page, where #474747 read 9.29:1 and
 * 8.59:1; #72716b reads 4.65:1 and 4.21:1, where #767676 read 4.54:1 and
 * 4.20:1.
 */
export const INK_SOFT = '#44423d'
export const INK_FAINT = '#72716b'

export const ACCENT = '#1a4ed8'

/** The rule between things, which is the paper one step down from the sheet. */
export const HAIRLINE = '#e0ddd3'

/**
 * The slab and the two inks cut for it.
 *
 * The slab is warm charcoal rather than black and the ink on it is the page's
 * own ivory. Lifting the ground off black costs everything drawn on it some
 * contrast, so the muted step is a lighter step of that ivory: #b0aea5 reads
 * 7.48:1 on the slab where #9a9a9a read 7.46:1 on black, which the annotation
 * needs because it is the smallest type in the family.
 */
const SLAB = '#1f1e1d'
const SLAB_INK = '#f0eee6'
const SLAB_INK_MUTE = '#b0aea5'
 ...
 * flipping the cell leaves pale ink on pale paper.
 */
...
function lockup(brand, light) {
  ...
  const named = `<td valign="middle" style="font-family:${MONO};font-size:15px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;line-height:1.3;color:${light ? INK : SLAB_INK};">${escapeHtml(brand.name)}</td>`
  if (!mark) {
    return `<td align="left" valign="middle" style="font-family:${MONO};font-size:15px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;line-height:1.3;color:${light ? INK : SLAB_INK};">${escapeHtml(brand.name)}</td>`
  }
 ...
 *
 * The dark slab is the strongest thing on the sheet and it is drawn for the
 ...
 * on recorded beside it, and half of them are dark artwork that disappears on a
 * charcoal ground. So a brand says which ground it needs, and one that needs a
 * pale one opens on the card itself with a hairline under it instead. The
 * alternative is a masthead that is empty for exactly the clients whose logo is
 * dark ink.
 *
