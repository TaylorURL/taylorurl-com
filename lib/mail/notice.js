...
/**
 * A single-line field, as somebody else sent it, capped. Line breaks come out
 * because these values reach the subject and the Reply-To header, where a break
 * is a way to write a header of one's own.
 *
 * @param {unknown} value
 * @param {number} limit
 ...
 * @param {unknown} value
 * @param {number} limit
 * @returns {string} The block, or an empty string.
 */
export function block(value, limit) {
  if (typeof value !== 'string') return ''
