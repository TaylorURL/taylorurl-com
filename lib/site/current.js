 ...
 * `scripts/check-site-key.js` fails the build if that stops being true. The
 ...
 * paths — and 59 of the 63 check scripts run as plain `node scripts/check-x.js`
 ...
 * to prevent. The repo already answers this way: `src/app/utils/emailTemplate.js`
 * reaches the mail modules as `../../../lib/mail/*.js` for exactly this reason.
 */
 ...
 * `scripts/check-site-key.js` resolves every registered key in a child process
 ...
 * `scripts/check-site-key.js` enforces it. After Vite substitutes the literal
