/**
 * A row in `start_leads`, and the reading and stamping every caller does of one.
 *
 * The table is one row per address. Nothing writes a new one now, and the rows
 * already in it are worked: the console lists them, answers them and takes an
 * address off the list, and this is what says whether a given address is one a
 * message can be sent to at all.
 *
 * What happens to a lead is written as times rather than as a stage. A lead can
 * be enquired and bought, or bought without ever enquiring, and a single column
 * holding a word would have to pick one of those to forget. Each fact stands on
 * its own and reading them together is the caller's business.
 *
 * The readers here are shared with `spine.js`, which is the table every door
 * writes to now. A field measured two ways is a field the two tables disagree
 * about, so the measuring is written once and both read it.
 */
...
const LIMITS = { email: 254, tag: 200 }
...
export function line(value, limit) {
  if (typeof value !== 'string') return null
...
/**
 * What a form held, read back as rows a console can draw.
 *
 * It arrives from a browser and is read as such: anything that is not a pair of
 * strings is not a row, both halves are trimmed to a length something has to
 * render, and rows past the cap are dropped rather than stored. A brief is what
 * a person reads to decide whether to write back; it is not a place for
 * whatever a stranger cared to post.
 *
 * @param {unknown} held
 ...
 * joining tables, because the moments leave no other trace under the address:
 * an enquiry is delivered as mail and files no name.
 *
