...
import { briefFrom, campaignFrom, line, usableEmail } from './record.js'
 ...
 *
 * `configurator` and `payment` are doors nothing writes any more. The rows they
 * wrote are still in the table and still worked, so the spellings stay: a
 * source dropped from here is a filter the console cannot offer and a lead
...
export async function keepLead(
  ...
  const { data, error } = await db.rpc('lead_record', {
    ...
    p_email: address,
    p_name: line(name, LIMITS.line),
    p_phone: line(phone, LIMITS.line),
    p_business: line(business, LIMITS.line),
    p_trade: line(trade, LIMITS.line),
    p_website: line(website, LIMITS.line),
    p_town: line(town, LIMITS.line),
    p_note: block(note),
