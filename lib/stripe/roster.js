 ...
 */
export const ROSTER_KINDS = ['not_client', 'setup_invoice', 'off_stripe']

/**
 * A roster holding nothing.
 *
 * What a test builds on, and what a caller must never quietly fall back to: an
 * empty roster reports the studio’s own card as a client and reads an
 * up-front fee as a monthly one. Wrong figures on a money screen are worse
 * than no screen, so a caller that cannot load the roster refuses rather than
 * substitutes this.
 *
