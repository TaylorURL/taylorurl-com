...
// Errors generated by the TaylorURL infrastructure itself (analytics beacon,
// error reporter) must never be re-reported — that creates feedback loops where
// every failed heartbeat or batch send becomes a fresh GitHub issue. The new
// beacon already filters these client-side, but legacy clients in the wild
// (sites that haven't redeployed) need a server-side guard.
const SELF_REFERENTIAL_PATTERNS = [
    /\banalytics-service\b/i,
    /\berror-reporting-service\b/i,
    /\bbeacon\.js\b/i,
    /\b__BASE_URL__\b/,
]

function isSelfReferential(record: Record<string, unknown>): boolean {
    // Deliberately excludes `url` (the page URL) — a real user-visible error on
    // a page like /admin/analytics-service/ would otherwise be silently dropped.
    const fields = ["error_message", "source_file", "stack_trace"]
...
function validateErrorReport(entry: unknown): ErrorReport | null {
    ...
    if (isSelfReferential(record)) return null
