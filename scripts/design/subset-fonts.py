...
against a freshly downloaded upstream build; commit the result.
    ...
    python3 scripts/design/subset-fonts.py <upstream-dir>
    python3 scripts/design/subset-fonts.py <upstream-dir> --check
...
scripts/design/check-font-coverage.js holds the result to all of that on every run of
...
def main() -> int:
    ...
    fonts = pathlib.Path(__file__).resolve().parent.parent.parent / 'public' / 'fonts'
