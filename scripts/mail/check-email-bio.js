...
if (stray.length) {
  fail(`outreach HTML draws ${stray.length} image(s) beyond the open pixel`)
}
...
if (!images.some(tag => /signature\.png/.test(tag) && tag.includes(`alt="${BIO_NAME},`))) {
  fail('outreach HTML is unsigned, or signs with a picture that says nothing')
}
...
for (const [what, html] of Object.entries(drawn)) {
  for (const source of html.matchAll(/src="([^"]+)"/g)) {
    ...
    if (bad) fail(`${what} draws a ${bad} image, which mail clients do not: ${source[1]}`)
  }
}

await finish({ hint: 'the wording lives in lib/mail/bio.js' })
