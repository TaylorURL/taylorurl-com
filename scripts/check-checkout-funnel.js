...
function everySource(from = 'src', held = []) {
  for (const name of readdirSync(join(ROOT, from))) {
    const path = `${from}/${name}`
    if (statSync(join(ROOT, path)).isDirectory()) everySource(path, held)
    else if (/\.(jsx?|tsx?)$/.test(name)) held.push(path)
  }
  return held
}
...
check('the funnel every buy button leads to carries the payment step', () => {
  const start = read('src/app/views/Start.jsx')
  ...
  same(
    read('src/app/views/start/PaySection.jsx').includes('onSubmit'),
    true,
  ...
  const banner = read('src/app/components/CtaBanner.jsx')
...
check('the price the checkout charges is the price the pages print', () => {
  const pricing = read('src/app/data/pricing.js')
