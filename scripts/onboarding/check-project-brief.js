/**
 * Proves a paid build can be found again by whoever paid for it, and that the
 * answers stored against it reach the studio.
 *
 * A build attaches to the account whose address matches the one that paid,
 * exactly. Nobody types that address - the account is opened against it by the
 * payment itself - but a build can still be pointed at an address whose owner
 * is looking somewhere else, so the screen after a payment has to carry the
 * session forward to name it, and the console has to say something better than
 * nothing when a console has no build in it. Every step of that is silent when
 * it breaks: the payment clears, the screen draws, and the buyer simply never
 * finds their build.
 *
 * The brief a session names is read for the rest of its route rather than at
 * one end: named to Stripe, read back, attached to the project and drawn on the
 * build. Any link missing is the same lost brief.
 *
...
check('a buyer is carried back to the address they paid with', () => {
  same(read(LINK).includes('claimReturnUrl(SITE_URL'), true, 'the checkout sets a return address')
  same(
